...
ここでは、私たちはシンプルな`ERC20`フォーセットを実装します。ロジックは非常にシンプルです：`ERC20`トークンをフォーセットコントラクトに送り、ユーザーはコントラクトの`requestTokens()`関数を呼び出して`100`単位のトークンを受け取ることができます。各アドレスは 1 回だけ受け取ることができます。
